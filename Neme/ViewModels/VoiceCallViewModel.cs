    ...
    public class VoiceCallViewModel : BaseViewModel
    {
        ...
        public async Task StartVoiceCall(string recipient)
        {
           ...
           // await _peerConnection.SetLocalDescriptionAsync(offer);
            await _signalingClient.SendSdp(CallParticipant, content, "offer");
        ...
        private async void HandleSdpMessage(string sender, string sdp, string type)
        {
            ...
            if (type == "offer")
            {
                ...
                if (answer)
                    content = "Answer";
                await _signalingClient.SendSdp(sender, content, "answer"); // No need to set local description manually
            }
            else if (type == "answer")
            {
        ...
        {
            await _signalingClient.AcceptCall(CallParticipant);
